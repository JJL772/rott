...
spriteevent* SpawnCinematicSprite(char* name, int duration, int numframes, int framedelay, int x, int y, int scale, int endx, int endy, int endscale)
{
	spriteevent* sprite;
	patch_t*     p;
...
=================
*/
void PrecacheBack(backevent* back) { W_CacheLumpName(back->name, PU_CACHE, CvtNull, 1); }
...
void PrecachePalette(paletteevent* event) { W_CacheLumpName(event->name, PU_CACHE, CvtNull, 1); }
...
=================
*/
void DrawBlankScreen(void) { VL_ClearVideo(0); }
