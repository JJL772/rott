...
statinfo stats[NUMSTATS] = {{0, SPR0_YLIGHT, stat_ylight, FL_LIGHT | FL_SHOOTABLE, 0, 0, 2, 0, 0},
...
{
	if (IsLight(tilex - 1, tiley) || IsLight(tilex - 1, tiley - 1) || IsLight(tilex, tiley - 1) || IsLight(tilex + 1, tiley - 1) ||
	    IsLight(tilex + 1, tiley) || IsLight(tilex + 1, tiley + 1) || IsLight(tilex, tiley + 1) || IsLight(tilex - 1, tiley + 1))
		return (false);
	else
		return (true);
}
...
boolean TurnOffLight1(int tilex, int tiley, int i, int j)
{
	...
	if (IsLight(tilex + i, tiley) || IsLight(tilex + i, tiley + j) || IsLight(tilex, tiley + j) || IsLight(tilex, tiley + tempy) ||
	    IsLight(tilex + i, tiley + tempy) || IsLight(tilex + tempi, tiley + tempy) || IsLight(tilex + tempi, tiley + j) ||
...
boolean TurnOffLight2(int tilex, int tiley, int j)
{
	...
	if (IsLight(tilex - 1, tiley) || IsLight(tilex - 1, tiley + j) || IsLight(tilex - 1, tiley + tempy) || IsLight(tilex, tiley + j) ||
	    IsLight(tilex, tiley + tempy) || IsLight(tilex + 1, tiley) || IsLight(tilex + 1, tiley + j) || IsLight(tilex + 1, tiley + tempy))
		return (false);
	else
		return (true);
}
...
boolean TurnOffLight3(int tilex, int tiley, int i)
{
	...
	if (IsLight(tilex, tiley - 1) || IsLight(tilex + 1, tiley - 1) || IsLight(tilex + tempx, tiley - 1) || IsLight(tilex + i, tiley) ||
	    IsLight(tilex + tempx, tiley) || IsLight(tilex, tiley + 1) || IsLight(tilex + i, tiley + 1) || IsLight(tilex + tempx, tiley + 1))
		return (false);
	else
		return (true);
}
...
void SpawnStatic(int tilex, int tiley, int mtype, int zoffset)
{
	...
	if (temp)
	{
		...
		onetimer = ((mtype == stat_rubble) || (mtype == stat_woodfrag) || (mtype == stat_metalfrag) || (mtype == stat_missmoke));
		...
		if ((temp->shapenum != NOTHING) && (temp->flags & FL_ACTIVE))
		{
			...
			while (temp->ticcount <= 0)
			{
				if (temp->count < temp->numanims)
				{
					...
					if (((index == stat_rubble) && (temp->count == 9)) || ((index == stat_woodfrag) && (temp->count == 13)) ||
