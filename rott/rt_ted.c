...
void SortPreCache(void) { hsort((char*)cachelist, cacheindex, sizeof(cachetype), &CompareTags, &SwitchCacheEntries); }
...
int GetNextMap(int tilex, int tiley)
{
	...
	if (((icon != EXITTILE) && (icon != SECRETEXITTILE)) || (((next & 0xff00) != 0xe200) && ((next & 0xff00) != 0xe400)))
	{
		int i, j;
...
void SetupTeams(void)
{
	...
	if (gamestate.battlemode == battle_CaptureTheTriad)
	{
		...
		for (i = 0; i < NUMSPAWNLOCATIONS; i++)
		{
			...
			if (CheckTile(sx, sy) && (!IsPlatform(sx, sy)) && (Number_of_Empty_Tiles_In_Area_Around(sx, sy) > TEAM[0].nummembers))
			{
...
void SetupPlayers(void)
{
	...
	if (gamestate.battlemode == battle_Tag)
	{
		...
		for (i = 0; i < numplayers; i++)
		{
			...
			{
				pstate->missileweapon = pstate->oldweapon = pstate->new_weapon = pstate->oldmissileweapon = pstate->bulletweapon =
					pstate->weapon									  = -1;
			}
		}
	}
...
void LinkActor(objtype* ob, int tilex, int tiley, void (*action)(long), void (*swapaction)(long))
{
	...
	if (!clocklinked)
	{
		...
		else if (touchindices[touchx][touchy])
		{
			...
			Error("tried to link an object at x=%d y=%d to a non-existent touchplate supposedly at x=%d y=%d", tilex, tiley, touchx,
			      touchy);
	}
