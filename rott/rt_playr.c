...
int buttonscan[NUMBUTTONS] = {sc_Control,   sc_Alt, sc_RShift,	sc_Space,      sc_PgUp,	     sc_PgDn,	   sc_Enter, sc_Delete, sc_Home,
			      sc_End,	    sc_1,   sc_2,	sc_3,	       sc_4,	     sc_CapsLock,  sc_F12,   sc_Comma,	sc_Period,
			      sc_BackSpace, sc_A,   sc_UpArrow, sc_RightArrow, sc_DownArrow, sc_LeftArrow, sc_Tab,   sc_T,	sc_Z};
...
williamdidthis FREE = {
	84, 5, 0,
	0,  9, {{done, 2, 1}, {done, 2, 2}, {done, 2, 3}, {done, 2, 4}, {done, 2, 5}, {done, 2, 6}, {done, 2, 7}, {done, 2, 8}, {reset, 2, 9}}};
...
williamdidthis WEAPONS[MAXWEAPONS] =
	...
	{{100, -1, 10, 0x2000l, 3, {{at_pulltrigger, 4, 1}, {done, 4, 2}, {reset, 4, 0}}}, // single pistol
	 {100,
	  ...
	  0x2000l,
	  6,
	  {{at_pulltrigger, 2, 1}, {done, 2, 2}, {done, 2, 3}, {at_pulltrigger, 2, 4}, {done, 2, 5}, {reset, 2, 3}}}, // double pistol
	 ...
	 {125,
	  ...
	  0x10000l,
	  7,
	  {{done, 3, 1}, {done, 3, 2}, {done, 3, 3}, {done, 3, 4}, {at_missileweapon, 3, 5}, {done, 3, 6}, {reset, 3, 7}}}, // GODHAND
	 ...
	 {200,
	  ...
	  0x10000l,
	  6,
	  {{done, 1, 1}, {done, 1, 2}, {at_pulltrigger, 1, 3}, {at_pulltrigger, 1, 4}, {at_pulltrigger, 1, 5}, {reset, 1, 6}}}, // BAT
...
void RevivePlayerobj(int tilex, int tiley, int dir, objtype* ob)
{
	...
	if (!gamestate.SpawnEluder)
	{
		...
		if (gamestate.battlemode == battle_Tag)
		{
			...
			{
				pstate->missileweapon = pstate->oldweapon = pstate->new_weapon = pstate->oldmissileweapon = pstate->weapon =
					wp_godhand;
				pstate->bulletweapon = -1;
...
===============
*/
void SetupBulletHoleLink(int num, statobj_t* item) { BulletHoles[num] = item; }
...

{
	...
	{
		SpawnMissile(ob, newmissiledata->obclass, newmissiledata->speed, ob->angle, newmissiledata->state, newmissiledata->offset);
...
void DogBlast(objtype* ob)
{
	...
	for (x = txl; x <= txh; x++)
		for (y = tyl; y <= tyh; y++)
		{
			...
			if (temp && (temp->which == ACTOR) && (temp->flags & FL_SHOOTABLE) && (temp != ob) && (temp->obclass < roboguardobj) &&
			    (temp->flags & FL_ABP))
			{
...
void Cmd_Use(objtype* ob)
{
	...
	else if ((doorn & 0x8000) && (!(doorn & 0x4000)))
	{
		...
		if (dptr->eindex != -1)
		{
			...
			if (((dnum == eptr->door1) && (eptr->state == ev_rad)) || ((dnum == eptr->door2) && (eptr->state == ev_ras)))
				if (ob == player)
	...
	else if ((tempactor) && (tempactor->which == ACTOR) && (tempactor->obclass == pillarobj) && DISTOK(ob->x, tempactor->x, TD) &&
		 DISTOK(ob->y, tempactor->y, TD) && (!(tempactor->flags & FL_DONE)) && (!MAPSPOT(tempactor->tilex, tempactor->tiley, 2)))
	...
	{
		tempwall->flags |= FL_S_FLIPPED;
		if ((tempwall->flags & FL_W_INVERTED) && DISTOK(ob->x, wallx, TD) && DISTOK(ob->y, wally, TD) && DISTOK(ob->z, 0, 32))
		{
...
void PollAssassin(void)
{
	...
	controlbuf[0] += -(FixedMulShift(SWIFTStatus.pitch, viewcos, 16)) + FixedMulShift(-SWIFTStatus.roll, costable[strafeAngle], 16);

	controlbuf[1] += FixedMulShift(SWIFTStatus.pitch, viewsin, 16) - FixedMulShift(-SWIFTStatus.roll, sintable[strafeAngle], 16);
...
void PollControls(void)
{
	...
	{
		if ((playerdead == true) && (buttonpoll[bt_strafe] || buttonpoll[bt_attack] || buttonpoll[bt_use] ||
...
boolean GivePlayerMissileWeapon(objtype* ob, playertype* pstate, statobj_t* check)
{
	...
	if ((GetWeaponForItem(check->itemnumber) == pstate->missileweapon) && (check->ammo == stats[check->itemnumber].ammo) &&
...
void Thrust(objtype* ob)
{
	...
	if ((gamestate.battlemode == battle_CaptureTheTriad) && (ob->flags & FL_DESIGNATED) && (ob->tilex == TEAM[pstate->team].tilex) &&
...
#define WHERE_PLAYER_DROPPED_WEAPON(ob, pstate) (((ob)->tilex == (pstate)->weaponx) && ((ob)->tiley == (pstate)->weapony))
			...
			if (OBJECT_IS_BONUS(tstat) && OK_TO_PICK_UP(tstat, pstate, ob) && (!WHERE_PLAYER_DROPPED_WEAPON(ob, pstate)))
			{
				GetBonus(ob, tstat);
...
void	      PlayerTiltHead(objtype* ob)
{
	...
	else
	{
		...
		if (!(ob->flags & FL_DOGMODE) && !(ob->flags & FL_GODMODE) && !(ob->flags & FL_FLEET) && !(ob->flags & FL_RIDING) &&
...
void PlayerMove(objtype* ob)
{
	...
	if (IsWindow(ob->tilex, ob->tiley))
	{
		if (!(ob->flags & FL_DYING))
		{
			pstate->health	= 0;
			pstate->falling = true;
			if ((ob->flags & FL_GODMODE) || (ob->flags & FL_DOGMODE) || (gamestate.battlemode == battle_Eluder) || (godmode == true))
			{
				KillActor(ob);
...
void T_Attack(objtype* ob)
{
	...
	if (pstate->attackframe >= WEAPONS[pstate->weapon].numattacks)
		Error("\n attackframe %d for weapon %d gt numattacks %d", pstate->attackframe, pstate->weapon, WEAPONS[pstate->weapon].numattacks);
...
void Switch_Who_Is_It_For_Tag(objtype* actor1, objtype* actor2)
{
	...
	if (pstate->buttonstate[bt_use])
	{
		...
		pstate2->oldmissileweapon = pstate2->oldweapon = pstate2->new_weapon = pstate2->missileweapon = pstate2->weapon = wp_godhand;
...
#define REGDOGSTATE(ob) ((ob->state == &s_serialdog) || (ob->state == &s_serialdog2) || (ob->state == &s_serialdog3) || (ob->state == &s_serialdog4))
...
void T_Player(objtype* ob)
{
	...
	if (ob->flags & FL_DYING)
	{
		...
		CheckPlayerSpecials(ob);
		PlayerMove(ob);
		if ((pstate->falling == true) || ((!ob->momentumx) && (!ob->momentumy) && (!ob->momentumz) && (!ob->state->tictime)))
		{
			KillActor(ob);
	...
	else if (((pstate->weapon == wp_bat) || ((pstate->weapon == wp_dog) && (!eluder))) && (pstate->buttonheld[bt_attack]))
	{
		if (pstate->weapon == wp_bat)
