...
static void ReadUDPPacket() { rottcom->remotenode = -1; }
...
void ComSetTime(void)
{
	...
	if (((networkgame == true) && (IsServer == true)) || ((networkgame == false) && (consoleplayer == 0))) // Master/Server
	{
		int nump;
		int time;
...
void SetTransitTime(int client, int time) { transittimes[client] = time; }
...
int GetTransitTime(int client) { return transittimes[client]; }
