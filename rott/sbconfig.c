...
static char* SbButtonNames[] = {"BUTTON_A", "BUTTON_B", "BUTTON_C", "BUTTON_D", "BUTTON_E", "BUTTON_F"};
...
int SbConfigParse(char* _filename)
{
	...
	if (!_filename)
		_filename = DEFAULT_CONFIG_FILENAME;
...
fixed SbFxConfigWarp(WarpRecord* warp, short value)
{
	...
	for (i = 0; i < warp->nWarp; i++)
	{
		...
		{
			fixed partial;
			partial = FIXED_MUL(INT_TO_FIXED((long)warp->pWarp[i].high - (long)warp->pWarp[i].low), warp->pWarp[i].mult);
			accum	= FIXED_ADD(accum, partial);
		}
	}
