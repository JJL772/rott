...
static void (*touchactions[NUMTOUCHPLATEACTIONS])(long) = {ActivatePushWall, ActivateMoveWall, LinkedOpenDoor, LinkedCloseDoor,
							   EnableObject,     DisableObject,    ActivateLight,  DeactivateLight};

#if ((DEVELOPMENT == 1))
#if ((LOADSAVETEST == 1))
static char* touchstrings[NUMTOUCHPLATEACTIONS] = {"ActivatePushWall", "ActivateMoveWall", "LinkedOpenDoor", "LinkedCloseDoor",
						   "EnableObject",     "DisableObject",	   "ActivateLight",  "DeactivateLight"};
#endif
#endif
#if ((DEVELOPMENT == 1))
...
static char* elevstring[NUMELEVATORACTIONS] = {"ready at source", "ready at destination", "moving to source", "moving to destination", "doorclosing"};
#endif
#endif
...
void LinkedOpenDoor(long door) { UtilizeDoor(door, OpenDoor); }
...
#define SHOULD_START_ELEVATOR_MUSIC(eptr)                                                                                                            \
	((demoplayback == false) && (demorecord == false) && (MusicStarted() == true) && (!BATTLEMODE) && (!(player->flags & FL_GODMODE)) &&         \
