...

{
	...
	if (SHOW_TOP_STATUS_BAR())
	{
		...
		else if (iGLOBAL_SCREENWIDTH == 800)
		{
			...
			// delete hart in middle of topbar
			DrawPPic(480 + 3, 1, 8 >> 2, 16, (byte*)&erase->data, 2, true, bufferofsonly);
			// delete bullet in end of topbar
			DrawPPic(780, 1, 8 >> 2, 16, (byte*)&erase->data, 2, true, bufferofsonly);
			shape = (pic_t*)W_CacheLumpName("stat_bar", PU_CACHE, Cvt_pic_t, 1);
			GameMemToScreen(shape, 0, 0, bufferofsonly);
		}
	...
	if (SHOW_BOTTOM_STATUS_BAR())
	{
		...
		// else
		{
			...
			if (iGLOBAL_SCREENWIDTH == 640)
			{
				...
				DrawPPic(310, (224 * 2) + 17 - ShowKillsYoffset, 8 >> 2, 16, (byte*)&erase->data, 2, true, bufferofsonly);
				// delete hart in middle of shape picture
				DrawPPic(324, (224 * 2) + 17 - ShowKillsYoffset, 8 >> 2, 16, (byte*)&erase->data, 2, true, bufferofsonly);
			}
			else if (iGLOBAL_SCREENWIDTH == 800)
			{
				...
				DrawPPic(305, 584 + 1 - ShowKillsYoffset, 8 >> 2, 16, (byte*)&erase->data, 2, true, bufferofsonly);
				// delete hart in middle of shape picture
				DrawPPic(610, 584 + 1 - ShowKillsYoffset, 8 >> 2, 16, (byte*)&erase->data, 2, true, bufferofsonly);
			}
			else
			{
		...
		if (demoplayback)
		{
			...
			{
				DrawPPic(380, 585, shape->width, shape->height, (byte*)&shape->data, 1, true, bufferofsonly);
			}
			else
			{
	...
	if (locplayerstate->poweruptime)
	{
		...
		DrawMPPic(POWERUP1X, POWERUPY + powerupheight, shape->width, shape->height - powerupheight, powerupheight, (byte*)&shape->data,
			  bufferofsonly);
	}
	...
	if (locplayerstate->protectiontime)
	{
		...
		DrawMPPic(POWERUP2X, POWERUPY + protectionheight, shape->width, shape->height - protectionheight, protectionheight,
...
void DrawKills(boolean bufferofsonly)
{
	...
	if (SHOW_TOP_STATUS_BAR())
	{
		...
		if (((gamestate.battlemode == battle_Tag) || (gamestate.battlemode == battle_Hunter)) && (BATTLE_It == BATTLE_Team[consoleplayer]))
		{
...
void GiveMissileWeapon(objtype* ob, int which)
{
	...
	if (!gamestate.BattleOptions.WeaponPersistence)
	{
		if (pstate->ammo && (pstate->missileweapon != -1) && (!(WEAPON_IS_MAGICAL(which))) && (!(WEAPON_IS_MAGICAL(pstate->missileweapon))))
		{
			int nx, ny;
...

{
	...
	{
		DrawPPic(iGLOBAL_HEALTH_X, health_y, 8 >> 2, 16, (byte*)&health[0]->data, oldpercenthealth, true, bufferofsonly);
	}
	else if (oldpercenthealth < 5)
	{
		DrawPPic(iGLOBAL_HEALTH_X, health_y, 8 >> 2, 16, (byte*)&health[1]->data, oldpercenthealth, true, bufferofsonly);
	}
	else
	{
		DrawPPic(iGLOBAL_HEALTH_X, health_y, 8 >> 2, 16, (byte*)&health[2]->data, oldpercenthealth, true, bufferofsonly);
	}

	if (oldpercenthealth < 10)
	{
		DrawPPic(iGLOBAL_HEALTH_X + (8 * oldpercenthealth), health_y, 8 >> 2, 16, (byte*)&erase->data, 10 - oldpercenthealth, true,
			 bufferofsonly);
	}
}
...

{
	...
	{
		DrawPPic(iGLOBAL_AMMO_X, ammo_y + 1, 8 >> 2, 16, (byte*)&ammo[locplayerstate->new_weapon]->data, locplayerstate->ammo, false,
			 bufferofsonly);
	}
}
	...
	if (GamePaused == true)
	{
		...
		/*
			 ...
			 // memcpy(tmpPICbuf,bufftemp,iGLOBAL_SCREENWIDTH*iGLOBAL_SCREENHEIGHT);
			  ...
			  //VL_MemStrechedToScreen ((byte *)&p->data, p->width, p->height,(iGLOBAL_SCREENWIDTH-((p->width*2)<<2) ) >>1,
...
//
//******************************************************************************
void SaveTag(int handle, char* tag, int size) { SafeWrite(handle, tag, size); }
