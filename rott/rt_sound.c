...
#ifdef DOS
int musicnums[11] = {-1, UltraSound, SoundBlaster, SoundMan16, ProAudioSpectrum, Awe32, SoundScape, WaveBlaster, GenMidi, SoundCanvas, Adlib};

int fxnums[11] = {-1, UltraSound, SoundBlaster, SoundMan16, ProAudioSpectrum, Awe32, SoundScape, Adlib, SoundSource, TandySoundSource, PC};
#else
...
void MUSIC_SetPosition(int time) { MUSIC_SetSongTime((unsigned long)time); }
...
int SD_PlayIt(int sndnum, int angle, int distance, int pitch)
{
	...
	if (*snd == 'C')
	{
		voice = FX_PlayVOC3D(snd, pitch, angle, distance, sounds[sndnum].priority, (unsigned long)sndnum);
	}
	else
	{
...

{
	...
	HandleMultiPageCustomMenu(SongNames, MAXSONGS, lastsongnumber, "Jukebox", MU_PlayJukeBoxSong, MU_JukeBoxRedraw, false);
...
//
//***************************************************************************
boolean MusicStarted(void) { return MU_Started; }
...
int MU_GetSongNumber(void) { return lastsongnumber; }
