...
#define ADLIBFX_SetErrorCode(status) ADLIBFX_ErrorCode = (status);
...
int ADLIBFX_GetTotalVolume(void) { return (ADLIBFX_TotalVolume); }
...
static void ADLIBFX_LockEnd(void) {}
...
void ADLIBFX_SetCallBack(void (*function)(unsigned long)) { ADLIBFX_CallBackFunc = function; }
