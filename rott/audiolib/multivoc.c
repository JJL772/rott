...
#define RoundFixed(fixedval, bits) (((fixedval) + (1 << ((bits)-1))) >> (bits))
		...
		while (length > 0)
		{
			...
			if (MV_Bits == 16)
			{
				if (MV_ReverbTable != NULL)
				{
					...
					if ((MV_SoundCard == UltraSound) && (MV_Channels == 2))
					{
						MV_16BitReverb(source + MV_RightChannelOffset, dest + MV_RightChannelOffset,
				...
				else
				{
					...
					if ((MV_SoundCard == UltraSound) && (MV_Channels == 2))
					{
						MV_16BitReverbFast(source + MV_RightChannelOffset, dest + MV_RightChannelOffset, count / 2,
								   MV_ReverbLevel);
					}
				}
			}
			else
			{
				if (MV_ReverbTable != NULL)
				{
					...
					if ((MV_SoundCard == UltraSound) && (MV_Channels == 2))
					{
						MV_8BitReverb(source + MV_RightChannelOffset, dest + MV_RightChannelOffset,
				...
				else
				{
					...
					if ((MV_SoundCard == UltraSound) && (MV_Channels == 2))
					{
						MV_8BitReverbFast(source + MV_RightChannelOffset, dest + MV_RightChannelOffset, count,
								  MV_ReverbLevel);
					}
				}
			}
...

{
	...
	while (!done)
	{
		...
		case 0:
			// End of data
			if ((voice->LoopStart == NULL) || ((unsigned char*)voice->LoopStart >= (ptr - 4)))
			{
				voice->Playing = FALSE;
				done	       = TRUE;
			}
...
int MV_GetReverbDelay(void) { return MV_ReverbDelay / MV_SampleSize; }
...

{
	...
	case SoundBlaster:
	case Awe32:
		status = BLASTER_BeginBufferedPlayback(MV_MixBuffer[0], TotalBufferSize, MV_NumberOfBuffers, MV_RequestedMixRate, MV_MixMode,
						       MV_ServiceVoc);
	...
	case UltraSound:
		...
		if (MV_Channels == 2)
		{
			status = GUSWAVE_StartDemandFeedPlayback(MV_ServiceRightGus, 1, MV_Bits, MV_RequestedMixRate, 0, 8, 255, 0xffff, 0);
			if (status < GUSWAVE_Ok)
			{
	...
	case ProAudioSpectrum:
	case SoundMan16:
		status = PAS_BeginBufferedPlayback(MV_MixBuffer[0], TotalBufferSize, MV_NumberOfBuffers, MV_RequestedMixRate, MV_MixMode,
						   MV_ServiceVoc);
	...
	case SoundScape:
		status = SOUNDSCAPE_BeginBufferedPlayback(MV_MixBuffer[0], TotalBufferSize, MV_NumberOfBuffers, MV_RequestedMixRate, MV_MixMode,
							  MV_ServiceVoc);
	...
	case SoundSource:
	case TandySoundSource:
		SS_BeginBufferedPlayback(MV_MixBuffer[0], TotalBufferSize, MV_NumberOfBuffers, MV_ServiceVoc);
...
#else
	status = DSL_BeginBufferedPlayback(MV_MixBuffer[0], TotalBufferSize, MV_NumberOfBuffers, MV_RequestedMixRate, MV_MixMode, MV_ServiceVoc);
...

{
	...
	case ProAudioSpectrum:
	case SoundMan16:
		status = PAS_BeginBufferedRecord(MV_MixBuffer[0], TotalBufferSize, NumberOfBuffers, MixRate, MONO_8BIT, MV_ServiceRecord);
...
int MV_StartDemandFeedPlayback(void (*function)(char** ptr, unsigned long* length), int rate, int pitchoffset, int vol, int left, int right,
...
int MV_PlayLoopedRaw(char* ptr, unsigned long length, char* loopstart, char* loopend, unsigned rate, int pitchoffset, int vol, int left, int right,
		     int priority, unsigned long callbackval)
...
void MV_SetCallBack(void (*function)(unsigned long)) { MV_CallBackFunc = function; }
...
void MV_SetReverseStereo(int setting) { MV_SwapLeftRight = setting; }
...
int MV_GetReverseStereo(void) { return (MV_SwapLeftRight); }
