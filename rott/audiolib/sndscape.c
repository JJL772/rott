...
const int SOUNDSCAPE_Interrupts[SOUNDSCAPE_MaxIrq + 1] = {INVALID, INVALID, 0xa,  INVALID, INVALID, 0xd,     INVALID, 0xf,
							  INVALID, INVALID, 0x72, INVALID, INVALID, INVALID, INVALID, INVALID};

const int SOUNDSCAPE_SampleSize[SOUNDSCAPE_MaxMixMode + 1] = {MONO_8BIT_SAMPLE_SIZE, STEREO_8BIT_SAMPLE_SIZE, MONO_16BIT_SAMPLE_SIZE,
...
#define SOUNDSCAPE_SetErrorCode(status) SOUNDSCAPE_ErrorCode = (status);
...
unsigned SOUNDSCAPE_GetPlaybackRate(void) { return (SOUNDSCAPE_SampleRate); }
...

{
	...
	offset = (int)(((unsigned long)CurrentAddr) - ((unsigned long)SOUNDSCAPE_CurrentDMABuffer));
...
int SOUNDSCAPE_BeginBufferedPlayback(char* BufferStart, int BufferSize, int NumDivisions, unsigned SampleRate, int MixMode,
...

{
	...
	DmaStatus = SOUNDSCAPE_SetupDMABuffer(BufferStart, BufferSize, DMA_AutoInitRead);
...
void SOUNDSCAPE_SetCallBack(void (*func)(void)) { SOUNDSCAPE_CallBack = func; }
...

{
	...
	if (SOUNDSCAPE_Config.ChipID != MMIC)
	{
		...
		if (SOUNDSCAPE_Config.SBEmul)
		{
			ga_write(GA_INTCFG, (ga_read(GA_INTCFG) & 0xf3) | (SOUNDSCAPE_Config.IRQIndx << 2));
		}
