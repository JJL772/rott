...
void	    TextMode(void);
#pragma aux TextMode = "mov    ax, 0003h", "int    10h" modify[ax];
...
void MUSIC_SetSongTick(unsigned long PositionInTicks) { MIDI_SetSongTick(PositionInTicks); }
...
void MUSIC_SetSongTime(unsigned long milliseconds) { MIDI_SetSongTime(milliseconds); }
...

{
	...
	if ((card == WaveBlaster) || (card == SoundCanvas) || (card == GenMidi))
	{
...
static void MUSIC_FadeRoutine(task* Task)
...

{
	...
	if ((MUSIC_SoundDevice == ProAudioSpectrum) || (MUSIC_SoundDevice == SoundMan16) || (MUSIC_SoundDevice == GenMidi) ||
	    (MUSIC_SoundDevice == SoundScape) || (MUSIC_SoundDevice == SoundCanvas))
	{
...
int MUSIC_FadeActive(void) { return (MUSIC_FadeTask != NULL); }
...
void MUSIC_RerouteMidiChannel(int channel, int cdecl (*function)(int event, int c1, int c2)) { MIDI_RerouteMidiChannel(channel, function); }
