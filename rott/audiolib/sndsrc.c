...
#define SS_SetErrorCode(status) SS_ErrorCode = (status);
...
static void SS_ServiceInterrupt(task* Task)
...

{
	...
	offset = (int)(((unsigned long)SS_SoundPtr) - ((unsigned long)SS_CurrentBuffer));
...
int SS_GetPlaybackRate(void) { return (SS_SampleRate); }
...
void SS_SetCallBack(void (*func)(void)) { SS_CallBack = func; }
...
void SS_TestTimer(task* Task) { (*(int*)(Task->data))++; }
...
int SS_TestSoundSource(int port)
...

{
	...
	if ((soundcard == TandySoundSource) || (USER_CheckParameter(SELECT_TANDY_SOUNDSOURCE)))
	{
		// Tandy
		SS_OffCommand = 0x0e;
	}
