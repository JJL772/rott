...
static const int PAS_Interrupts[PAS_MaxIrq + 1] = {INVALID, INVALID, 0xa,  0xb,	 INVALID, 0xd,	   INVALID, 0xf,
...
#define PAS_SetErrorCode(status) PAS_ErrorCode = (status);
...

{
	...
	ptr = (MVState*)((((unsigned)regs.w.dx) << 4) + ((unsigned)regs.w.bx));
#else
...
#if defined(__WATCOMC__) && defined(__FLAT__)
	ptr = (MVFunc*)((((unsigned)regs.w.dx) << 4) + ((unsigned)regs.w.bx));
#else
	ptr = MK_FP(regs.w.dx, regs.w.bx);
#endif
...
unsigned PAS_GetPlaybackRate(void) { return (PAS_SampleRate); }
...

{
	...
	offset = (int)(((unsigned long)CurrentAddr) - ((unsigned long)PAS_CurrentDMABuffer));
...
#pragma aux PAS_CallInt = "int 2fh", parm[ebx][ecx][edx] modify exact[eax ebx ecx edx esi edi] value[ebx];
...

{
	...
	status = PAS_CallMVFunction(PAS_Func->SetMixer, volume, OUTPUTMIXER, L_PCM);
	if (status == PAS_Error)
	{
		return (status);
	}

	status = PAS_CallMVFunction(PAS_Func->SetMixer, volume, OUTPUTMIXER, R_PCM);
	if (status == PAS_Error)
	{
		return (status);
	}
...

{
	...
	leftvolume  = PAS_CallMVFunction(PAS_Func->GetMixer, 0, OUTPUTMIXER, L_PCM);
	rightvolume = PAS_CallMVFunction(PAS_Func->GetMixer, 0, OUTPUTMIXER, R_PCM);
...

{
	...
	leftvolume  = PAS_CallMVFunction(PAS_Func->GetMixer, 0, OUTPUTMIXER, L_FM) & 0xff;
	rightvolume = PAS_CallMVFunction(PAS_Func->GetMixer, 0, OUTPUTMIXER, R_FM) & 0xff;
...
void PAS_SetCallBack(void (*func)(void)) { PAS_CallBack = func; }
...

{
	...
	if (status != PAS_Error)
	{
		PAS_OriginalFMLeftVolume = PAS_CallMVFunction(PAS_Func->GetMixer, 0, OUTPUTMIXER, L_FM) & 0xff;

		PAS_OriginalFMRightVolume = PAS_CallMVFunction(PAS_Func->GetMixer, 0, OUTPUTMIXER, R_FM) & 0xff;
...

{
	if (PAS_Func)
	{
		PAS_CallMVFunction(PAS_Func->SetMixer, PAS_OriginalFMLeftVolume, OUTPUTMIXER, L_FM);
		PAS_CallMVFunction(PAS_Func->SetMixer, PAS_OriginalFMRightVolume, OUTPUTMIXER, R_FM);
	}
}
...

{
	...
	PAS_OriginalPCMLeftVolume  = PAS_CallMVFunction(PAS_Func->GetMixer, 0, OUTPUTMIXER, L_PCM) & 0xff;
	PAS_OriginalPCMRightVolume = PAS_CallMVFunction(PAS_Func->GetMixer, 0, OUTPUTMIXER, R_PCM) & 0xff;
...

{
	...
	if (PAS_Installed)
	{
		...
		PAS_CallMVFunction(PAS_Func->SetMixer, PAS_OriginalPCMLeftVolume, OUTPUTMIXER, L_PCM);
		PAS_CallMVFunction(PAS_Func->SetMixer, PAS_OriginalPCMRightVolume, OUTPUTMIXER, R_PCM);
