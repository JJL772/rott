...
static int cdecl (*_MIDI_RerouteFunctions[NUM_MIDI_CHANNELS])(int event, int c1, int c2) = {NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
...
static int _MIDI_UserChannelVolume[NUM_MIDI_CHANNELS] = {256, 256, 256, 256, 256, 256, 256, 256, 256, 256, 256, 256, 256, 256, 256, 256};
...

{
	...
	case EMIDI_CONTEXT_END:
		if ((Track->currentcontext == _MIDI_Context) || (_MIDI_Context < 0) || (Track->context[_MIDI_Context].pos == NULL))
		{
			break;
		}
...

{
	...
	if (_MIDI_RerouteFunctions[channel] != NULL)
	{
		status = _MIDI_RerouteFunctions[channel](0xB0 + channel, c1, c2);
		if (status == MIDI_DONT_PLAY)
		{
			return (MIDI_Ok);
		}
...

{
	...
	if (_MIDI_RerouteFunctions[channel] != NULL)
	{
		...
		status = _MIDI_RerouteFunctions[channel](0xB0 + channel, MIDI_VOLUME, remotevolume);
		if (status == MIDI_DONT_PLAY)
		{
			return;
		}
...
int MIDI_GetContext(void) { return _MIDI_Context; }
...
int MIDI_SongPlaying(void) { return (_MIDI_SongActive); }
...

{
	...
	while ((tracknum < _MIDI_NumTracks) && (Track != NULL))
	{
		...
		if (RELATIVE_BEAT(_MIDI_Measure, _MIDI_Beat, _MIDI_Tick) > RELATIVE_BEAT(_MIDI_TotalMeasures, _MIDI_TotalBeats, _MIDI_TotalTicks))
		{
...

{
	...
	DPMI_UnlockMemory((void*)&_MIDI_CommandLengths[0], sizeof(_MIDI_CommandLengths));
...

{
	...
	status |= DPMI_LockMemory((void*)&_MIDI_CommandLengths[0], sizeof(_MIDI_CommandLengths));
