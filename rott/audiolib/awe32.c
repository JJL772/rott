...
static LONG lBankSizes[MAXBANKS] = {0};

unsigned    SetES(void);
#pragma aux SetES = "xor eax, eax"                                                                                                                   \
		    "mov ax, es"                                                                                                                     \
		    "mov bx, ds"                                                                                                                     \
		    "mov es, bx" modify[eax ebx];

void	    RestoreES(unsigned num);
#pragma aux RestoreES = "mov  es, ax" parm[eax];

int AWE32_ErrorCode = AWE32_Ok;

#define AWE32_SetErrorCode(status) AWE32_ErrorCode = (status);
...
static void AWE32_LockEnd(void) {}
