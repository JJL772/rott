...
#define CalcTimeInterval(rate) (1193180UL / (rate))

#define CalcSamplingRate(interval) (1193180UL / (interval))
...
#pragma aux PAS_TestAddress = "mov   dx, 0b8bh", "xor    dx, ax", "in    al, dx", "cmp   al, 0ffh", "je    TestExit", "mov   ah, al",                \
	    "xor   al, 0e0h", "out   dx, al", "jmp   TestDelay1", "TestDelay1:", "jmp   TestDelay2", "TestDelay2:", "in    al, dx", "xchg  al, ah",  \
	    "out   dx, al", "sub   al, ah", "TestExit:", "and   eax, 0ffh" parm[eax] modify exact[eax dx];
