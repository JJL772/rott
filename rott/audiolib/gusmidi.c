...

{
	...
	while (1)
	{
		...
		sscanf(text, "%d, %d, %d, %d, %d, %s\n", &ignore, &PatchMap[index][0], &PatchMap[index][1], &PatchMap[index][2], &PatchMap[index][3],
		       ProgramName[index]);
	}
...
int GUSMIDI_UnloadPatch(int prognum)
...
int GUSMIDI_LoadPatch(int prognum)

{
	...
	status = USRHOOKS_GetMem(&wave_buff, patchi.header.wave_forms * sizeof(struct wave_struct));
...
void GUSMIDI_ProgramChange(int channel, int prognum)
...
---------------------------------------------------------------------*/
#pragma warn - par
void GUSMIDI_NoteOff(int chan, int note, int velocity) { gf1_midi_note_off(note, chan); }
#pragma warn.par
...
void GUSMIDI_ControlChange(int channel, int number, int value) { gf1_midi_parameter(channel, number, value); }
...
void GUSMIDI_PitchBend(int channel, int lsb, int msb) { gf1_midi_pitch_bend(channel, lsb, msb); }
...
int GUSMIDI_GetVolume(void) { return (GUSMIDI_Volume); }
