...
void BATTLE_SetOptions(battle_type* options) { memcpy(&BattleOptions, options, sizeof(battle_type)); }
...
void BATTLE_GetOptions(battle_type* options) { memcpy(options, &BattleOptions, sizeof(battle_type)); }
...

{
	...
	{
		SoftError("Sorted rank %d = player %d : Score = %d\n", i, BATTLE_PlayerOrder[i], BATTLE_Points[BATTLE_PlayerOrder[i]]);
	}
#endif
...

{
	...
	if ((killerteam < 0) || (killerteam >= BATTLE_NumberOfTeams) || (victimteam < 0) || (victimteam >= BATTLE_NumberOfTeams))
	{
#if (BATTLECHECK == 1)
